...
## '두 모집단 평균 차이에 대한 검정' - 두 모집단의 "평균이 차이가 없다"는 기존 주장을 검정한다.
##                                        기존 주장에 배타적인 입장은 "평균에 차이가 없다고 할 수 없다."이다.
##    - 독립표본의 경우 : 표본들 사이에 관계나 인과가 없는 모집단 1과 2로부터 
##                        각각 n1, n2 개의 표본을 독립적으로 추출하여 두 모집단 표본평균간에 차이가 있는지를 검정한다.
##                        ex) 미래고등학교 3학년 120명과 노을고등학교 3학년 108명의 성적에 차이가 있는지를 검정
##    - 대응표본의 경우 : 학습이나 처리 등에 의해서 동일한 집단의 사전 사후 특성치가 달라지는지를 검정한다.
##                        표본통계량은 동일한 사람이나 물건을 대상으로 하므로 관찰대상의 사전사후 특성치를 서로 대응시켜 검정한다.
##                        ex) 120명의 미래고등학교 3학년 학생들이 EBS교육방송을 
##                            시청하기 전의 성적과 시청 후의 성적에 차이가 있는지를 검정
##    - 평균 = μ1 - μ2, 분산 = ∂1^2 / n1 + ∂2^2 / n2, 표준편차 = √(∂1^2 / n1 + ∂2^2 / n2)

### 1. 가설설정 - H0 : μ1 = μ2 or μ1 - μ2 = 0
###               H1 : μ1 ≠ μ2 or μ1 - μ2 ≠ 0, 보통 μ1 - μ2를 D로 표현한다.

### 2. 세 가지 가설검정 방법 - 등분산, 정규성, n의 크기를 고려한다.
### 1) Z-test(n >= 30) - 두 모집단의 '표준편차를 아는지 여부와 상관없이' 표본의 크기가 커서
###                      중심극한정리에 따라 모집단의 분포에 상관없이 두 집단 평균 차이를 검정.
###    - 신뢰구간 : (Xbar1 - Xbar2) ± Z α/2 √(∂1^2 / n1 + ∂2^2 / n2)
###    - 검정통계량(n >= 30) : Z = ((Xbar1 - Xbar2) - (μ1 - μ2)) / √(∂1^2 / n1 + ∂2^2 / n2)
###                              = ((Xbar1 - Xbar2) - (μ1 - μ2)) / √(S1^2 / n1 + S2^2 / n2)
### 2) 합동분산 t-test(n < 30) - 두 모집단의 '표준편차를 모르지만 동일하다는 가정', 
###    (pooled variance)         '정규분포를 따른다는 가정'하에 두 집단 평균차이 검정.
###    - 가정(등분산) : Sp^2 = ((n1 - 1) * S1^2 + (n2 - 1) * S2^2) / (n1 + n2 - 2)
###    - 신뢰구간 : (Xbar1 - Xbar2) ± t n1+n2-2, α/2 * √(Sp^2 * (1 / n1 + 1 / n2))
###    - 검정통계량 : t = ((Xbar1 - Xbar2) - (μ1 - μ2)) / √(Sp^2 * (1 / n1 + 1 / n2))
###    - 자유도 : df = n1 + n2 - 2
### 3) 이분산 t-test(n < 30) - 두 모집단의 '표준편차를 모르며, 정규분포를 따른다는 가정'하에 두 집단 평균 차이 검정
###    - 신뢰구간 : (Xbar1 - Xbar2) ± t df, α/2 * √(S1^2 / n1 + S2^2 / n2)
###    - 검정통계량(n < 30) : ((Xbar1 - Xbar2) - (μ1 - μ2)) / √(S1^2 / n1 + S2^2 / n2)
### ※ 표본평균을 두 개 사용하였으므로 t-test의 자유도를 '-1이 아닌 -2'로 변경
## 런던바게뜨 마케팅 분석 가설검정
## - (주)런던바게뜨는 전국에 120개의 점포를 두고 있다. 주택지역에 있는 점포 30군데와 
##   아파트단지 점포 36군데를 조사했더니 각각 37,233원(표준편차 3,475원), 35,855원(표준편차 2,580원)이었다.
##   지역에 따라 빵구매금액에 차이가 있는가 ? (유의수준 2%, 신뢰수준 98%)
## (1) 가설설정(양측검정) - H0 : μ1 - μ2 = 0 : 두 지역간 빵 구매금액에는 차이가 없다.
##                          H1 : μ1 - μ2 ≠ 0 : 두 지역간 빵 구매금액에는 차이가 있다.
## (2) 유의수준 - α = 0.02 두 지역간 빵 구매금액에 차이가 없음에도 불구하고 
##                차이가 있다고 결론을 내릴 확률이 0.02
## (3) 임계값과 기각영역의 산출 - 98% 신뢰구간의 Z 0.02/2 = Z 0.01은 ±2.326
## (4) 검정통계량의 계산 - ((Xbar1 - Xbar2) - 0) / √(∂1^2 / n1 + ∂2^2 / n2)
##                        = ((37,233 - 35,855) - 0) / √(3475^2 / 30 + 2580^2 / 36) = 1.7979
## (5) 임계치와 검정통계량의 비교 - 귀무가설의 98% 신뢰구간인 임계치 Z-value = ±2.326,
##                                  검정통계량은 +1.7979로 귀무가설의 98% 신뢰구간에 포함된다.
##                                  표준화 값 Z-value 대신 단위가 있는 실제값으로 임계치를 계산하면,
##                                  Zc = ((Xbar1 - Xbar2) - 0) / √(3475^2 / 30 + 2580^2 / 36) = ±2.326
##                                  임계값 ±2.326은 실제 단위로는 Xbar1 - Xbar2 = ±1782.991원에 해당하며,
##                                  검정통계량인 두 집단간 평균의 차이 766.433은 임계치를 안쪽에 위치한다.
## (6) 결론 - 따라서 런던바 두 지역간 빵 구매금액에는 차이가 있다고 볼 수 있다.
# t.test()함수를 이용
t.test(x, y = NULL, alternative = c("two.sided", "less", "greater"), mu = 0, paired = F,
       var.equal = F, conf.level = 0.95, ...) # Paired : 대응표본 여부, var.equal : 등분산 가정
## (주)런던바게뜨 마케팅 분석팀은 상세한 분석을 위해 조사 영역을 전국에서 대전지역으로 좁혔다.
## 대전지역 주택지역의 점포와 아파트단지 점포간 빵 구매금액에 차이가 있는지 분석하려 한다.
## 주택지역에 있는 점포 25군데와 아파트단지 점포 25군데를 조사했더니 
## 각각 36,468원(표준편차 4,938원) 34,056원(표준편차 1,615원)이었다.
## 두 집단간 빵 구매금액에 차이가 있는가 ? (유의수준 5%, 신뢰수준 95%)
# 등분산을 가정한 두 모집단 평균에 대한 가설검정
DF <- read.csv("H_test_mean_bread.csv", fileEncoding = "UTF-8")
x <- DF$Residential
y <- DF$APT
t.test(x, y, alternative = "two.sided", mu = 0, conf.level = 0.95, var.equal = T)
## 전국을 대상으로 분석한 결과와 달리 대전의 두 지역 1회 빵 구매금액이 같지 않음을 알 수 있다.
## 95% 신뢰수준에서 귀무가설을 기각한다.